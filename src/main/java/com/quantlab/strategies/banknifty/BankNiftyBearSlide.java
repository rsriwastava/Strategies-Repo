...
/**
 * BankNiftyBearSlide — Bear Put Spread on BANKNIFTY.
 *
 * <p>Structure: Buy 1 ATM PE + Sell 1 OTM PE (ATM - 300).
 * A net-debit directional strategy that profits from moderate downside movement.
 ...
 * @author QuantLab
 */
public class BankNiftyBearSlide extends BaseStrategy {

    private static final String STRATEGY_NAME = "BankNiftyBearSlide";
    ...
    /**
     * Constructs a new BankNiftyBearSlide strategy.
     *
     * @param orderService      the broker-agnostic order service
     * @param marketDataProvider the broker-agnostic market data provider
     */
    public BankNiftyBearSlide(OrderService orderService, MarketDataProvider marketDataProvider) {
        super(buildConfig());
    ...
    public void onEntry() {
        ...
        log("Entered BearSlide: bought {} PE @{}, sold {} PE @{}. Net debit={}, max profit={}",
    ...
    /**
     * Exits the entire Bear Put Spread position by squaring off all legs.
     */
    @Override
    public void onExit() {
        if (!positionOpen) {
        ...
        log("Exited BearSlide. Realised PnL={}", finalSnapshot.getRealizedPnl());
        positionOpen = false;
    }
