...
/**
 * BankNiftyPutDescent — Put Ladder (Long Put Ladder) on BANKNIFTY.
 *
 * <p>Structure:
 * <ul>
 *   <li>Buy 1 ATM PE</li>
 *   <li>Sell 1 OTM PE at ATM - 200</li>
 *   <li>Sell 1 far OTM PE at ATM - 400</li>
 * </ul>
 * Profits when the underlying stays in a range or moves moderately downward.
 * The two short puts reduce the cost of the long put but introduce
 * unlimited downside risk below the lowest strike. Net debit is typically
 * lower than a plain put spread.</p>
 *
 * <p>Exit conditions:</p>
 * <ul>
 *   <li>Target: 80% of max profit (occurs at ATM - 200 at expiry)</li>
 *   <li>Stop-loss: spot breaks below ATM - 400 by 100 points</li>
 ...
 * @author QuantLab
 */
public class BankNiftyPutDescent extends BaseStrategy {

    private static final String STRATEGY_NAME = "BankNiftyPutDescent";
    ...
    /**
     * Constructs a new BankNiftyPutDescent strategy.
     *
     * @param orderService      the broker-agnostic order service
     * @param marketDataProvider the broker-agnostic market data provider
     */
    public BankNiftyPutDescent(OrderService orderService, MarketDataProvider marketDataProvider) {
        super(buildConfig());
    ...
    /**
     * Enters the Put Ladder position.
     *
     * <p>Determines the ATM strike from the current spot price, then:
     * <ol>
     *   <li>Buys 1 lot ATM PE</li>
     *   <li>Sells 1 lot OTM PE at ATM - 200</li>
     *   <li>Sells 1 lot far OTM PE at ATM - 400</li>
     * </ol>
    ...
    public void onEntry() {
        ...
        double atmPremium = marketDataProvider.getOptionPremium(UNDERLYING, expiry, entryAtmStrike, OptionType.PE);
        double firstOtmPremium = marketDataProvider.getOptionPremium(UNDERLYING, expiry, firstOtmStrike, OptionType.PE);
        double farOtmPremium = marketDataProvider.getOptionPremium(UNDERLYING, expiry, farOtmStrike, OptionType.PE);
        ...
        log("Entered PutDescent: bought {} PE @{}, sold {} PE @{}, sold {} PE @{}. Net debit={}, max profit={}",
    ...
    /**
     * Exits the entire Put Ladder position by squaring off all legs.
     */
    @Override
    public void onExit() {
        if (!positionOpen) {
        ...
        log("Exited PutDescent. Realised PnL={}", finalSnapshot.getRealizedPnl());
        positionOpen = false;
    }
