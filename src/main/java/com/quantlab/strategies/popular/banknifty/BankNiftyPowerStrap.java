...
/**
 * BankNiftyPowerStrap — Strap strategy on BANKNIFTY.
 *
 * <p>Structure: Buy 2 ATM CE + Buy 1 ATM PE.
 * A bullish volatility play with CE:PE ratio of 2:1. The strategy profits
 * from a large move in either direction but has a greater reward on the
 * upside due to the extra call lot. Total premium paid is the net debit.</p>
 *
 * <p>Exit conditions:</p>
 * <ul>
 ...
 * @author QuantLab
 */
public class BankNiftyPowerStrap extends BaseStrategy {

    private static final String STRATEGY_NAME = "BankNiftyPowerStrap";
    ...
    private static final int CE_LOTS = 2;
    private static final int PE_LOTS = 1;
    ...
    /**
     * Constructs a new BankNiftyPowerStrap strategy.
     *
     * @param orderService      the broker-agnostic order service
     * @param marketDataProvider the broker-agnostic market data provider
     */
    public BankNiftyPowerStrap(OrderService orderService, MarketDataProvider marketDataProvider) {
        super(buildConfig());
    ...
    public void onEntry() {
        ...
        log("Entered PowerStrap: bought {}x {} CE @{} + {}x {} PE @{}. Total premium={}",
    ...
    /**
     * Exits the entire Strap position by squaring off all legs.
     */
    @Override
    public void onExit() {
        if (!positionOpen) {
        ...
        log("Exited PowerStrap. Realised PnL={}", finalSnapshot.getRealizedPnl());
        positionOpen = false;
    }
