...
/**
 * <b>NiftyPowerStrap</b> &mdash; Strap (bullish-biased long volatility).
 *
 * <p>Leg structure (2:1 CE:PE ratio):
 * <ul>
 *   <li>Buy 2 ATM CE</li>
 *   <li>Buy 1 ATM PE</li>
 * </ul>
 ...
 * with a bullish bias due to the 2:1 call-to-put ratio. Ideal before major
 * events or earnings when a large move is expected with an upside lean.
 *
 * <p>Max profit = unlimited (more on upside). Max loss = total premium paid.
 *
 * <p>Exit conditions:
 * <ol>
 ...
 * @author QuantLab
 */
public class NiftyPowerStrap extends BaseStrategy {

    private static final String STRATEGY_NAME = "NiftyPowerStrap";
    ...
    private static final int PE_QTY = 1;
    ...
    /**
     * Constructs a new NiftyPowerStrap strategy.
     *
     * @param orderService       broker-agnostic order service
     * @param marketDataProvider broker-agnostic market data provider
     */
    public NiftyPowerStrap(OrderService orderService, MarketDataProvider marketDataProvider) {
        super(buildConfig());
     ...
     * {@inheritDoc}
     *
     * <p>Buys 2 ATM CE and 1 ATM PE at the same strike to construct
     * the strap.
     */
    @Override
    public void onEntry() {
    ...
    public boolean shouldExit() {
        ...
        if (currentPnL >= totalPremium * STRAP_TARGET_PCT) {
            logger.info("{}: Target profit reached. PnL={}", STRATEGY_NAME, currentPnL);
            return true;
        }

        if (currentPnL <= -(totalPremium * STRAP_SL_PCT)) {
     ...
     * {@inheritDoc}
     *
     * <p>Squares off all open legs of the strap.
     */
    @Override
    public void onExit() {
        if (!positionOpen) {
