...
/**
 * <h2>SensexPutDescent &mdash; Put Ladder on SENSEX</h2>
 *
 * <p>A put ladder (also known as a long put ladder or bear put ladder) is
 * a three-legged strategy that starts as a bear put spread but adds an
 * additional short put at a lower strike. The extra short leg collects
 * premium to reduce cost but introduces unlimited downside risk below
 * the lowest strike.</p>
 *
 * <h3>Leg construction</h3>
 * <ul>
 *   <li><b>Leg 1</b> &ndash; Buy 1 ATM PE</li>
 *   <li><b>Leg 2</b> &ndash; Sell 1 OTM PE (ATM - 400)</li>
 *   <li><b>Leg 3</b> &ndash; Sell 1 far OTM PE (ATM - 800)</li>
 * </ul>
 *
 * <h3>Risk profile</h3>
 * <ul>
 ...
 *   <li>Upside risk is limited to the net debit paid.</li>
 *   <li>Downside risk is theoretically unlimited below the lowest short
 *       strike (net short one put below ATM - 800).</li>
 * </ul>
 *
 * @author QuantLab
 * @see BaseStrategy
 */
public class SensexPutDescent extends BaseStrategy {
    ...
    /**
     * Constructs a new {@code SensexPutDescent} strategy.
     *
     * @param orderService       broker-agnostic order execution service
     * @param marketDataProvider market data feed provider
     */
    public SensexPutDescent(OrderService orderService, MarketDataProvider marketDataProvider) {
        super(buildConfig());
     ...
     * {@inheritDoc}
     *
     * <p>Places the put ladder legs:
     * <ol>
     *   <li>Buys 1 ATM PE</li>
     *   <li>Sells 1 OTM PE at ATM - {@value #FIRST_OTM_OFFSET}</li>
     *   <li>Sells 1 far OTM PE at ATM - {@value #SECOND_OTM_OFFSET}</li>
     * </ol>
     */
    @Override
    public void onEntry() {
        ...
        Leg longPe = Leg.builder()
                .strike(atmStrike).optionType(OptionType.PE).side(OrderSide.BUY)
                .lots(LOTS_PER_LEG).expiry(expiry).segment(SEGMENT).exchange(EXCHANGE).build();

        Leg shortPe1 = Leg.builder()
                .strike(firstOtm).optionType(OptionType.PE).side(OrderSide.SELL)
                .lots(LOTS_PER_LEG).expiry(expiry).segment(SEGMENT).exchange(EXCHANGE).build();

        Leg shortPe2 = Leg.builder()
                .strike(secondOtm).optionType(OptionType.PE).side(OrderSide.SELL)
                .lots(LOTS_PER_LEG).expiry(expiry).segment(SEGMENT).exchange(EXCHANGE).build();

        Signal signal = Signal.builder()
                .strategyName(getConfig().getStrategyName())
                .legs(List.of(longPe, shortPe1, shortPe2))
                .build();

        orderService.placeOrder(signal);
        log("Entry placed — Put Ladder: Buy {} PE, Sell {} PE, Sell {} PE", atmStrike, firstOtm, secondOtm);
    }
